...
def forward_propagation_l5_2fc_nb(X, parameters, regularizer=None):
    ...
    W3 = parameters['W3']
    # -------------------------------------------------------------------------------------
    Z1 = tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME', use_bias=True)
    # RELU
    A1 = tf.nn.relu(Z1)
    P1 = tf.nn.avg_pool(A1, ksize=[1, 4, 4, 1], strides=[1, 2, 2, 1], padding='SAME')
    # -------------------------------------------------------------------------------------
    Z2 = tf.nn.conv2d(P1, W2, strides=[1, 2, 2, 1], padding='SAME', use_bias=True)
    # RELU
    A2 = tf.nn.relu(Z2)
    P2 = tf.nn.max_pool(A2, ksize=[1, 4, 4, 1], strides=[1, 4, 4, 1], padding='SAME')
    # -------------------------------------------------------------------------------------
    Z3 = tf.nn.conv2d(P2, W3, strides=[1, 1, 1, 1], padding='SAME', use_bias=True)
    # RELU
    A3 = tf.nn.relu(Z3)
